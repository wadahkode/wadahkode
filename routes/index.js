...
Router.get("/", (req, res) => {
  ...
  Model.tutorial.findAll("tutorials").then((snapshot) => {
    if (snapshot.length < 1) {
      ...
      snapshot.forEach((item) => {
        let createdAt = new Memories(),
          updatedAt = new Memories();

        createdAt.set("datetime", item.dibuat);
        updatedAt.set("datetime", item.diupdate);

        if (createdAt.timeAgo() == updatedAt.timeAgo()) {
          item.dibuat = createdAt.timeAgo();
          delete item.diupdate;
        } else {
          item.diupdate = updatedAt.timeAgo();
          delete item.dibuat;
        }
      });
